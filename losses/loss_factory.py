...
def create_loss(args, w1=1.0, w2=0.5):
    # Ensure compatibility with dictionary-style access
    conf_loss = args['base_loss'] if isinstance(args, dict) else args.base_loss
    print(f"Creating loss function: {conf_loss}")
    ...
    # Binary loss options
        ...
        raise ValueError(f"Invalid loss function: {conf_loss}")
...
if __name__ == '__main__':
    args = make_parse()
    myloss = create_loss(vars(args))  # Ensure compatibility by converting args to a dictionary
